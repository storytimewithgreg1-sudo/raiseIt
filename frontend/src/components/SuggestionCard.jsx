...
const SuggestionCard = ({suggestion}) => {
   ...
   const handleSuggestionDelete = async (e) => {
    ...
    return (
    <div className="card min-h-fit shadow-xl w-90 md:w-80 flex flex-col items-center justify-center bg-linear-to-r from-blue-400 to-purple-400 overflow-hidden">
        <div className="card-body w-full">
            <button onClick={handleSuggestionDelete} className='btn btn-xs btn-ghost text-white/60 hover:text-white hover:bg-transparent p-1 border-none self-end '>
                <X className='size-5 '/>
            </button>
            
            <h3 className="card-title  text-teal-100">{suggestion.title}</h3>
            <p className='text-teal-100/80 break-words'>{suggestion.content}</p>

            <div className="card-actions items-center justify-between">
               <button onClick={handlePinSuggestion} className='btn btn-xs rounded-full hover:bg-slate-200 btn border-none'>
                {suggestion.isPinned ?  <Pin className='text-sky-800 size-5 ' /> : <PinOff className='text-sky-800 size-5' />}
                </button>


                <div className='relative '>
                    <button onClick={handleVoteOnSuggestion} className='btn   rounded-full p-2 btn border-none bg-blue-600/30 hover:bg-blue-600/60  '>
